...
public class JwtServiceImpl implements JwtService{
    ...
    @Override
    public void updateRefreshToken(User user, RefreshToken refreshToken) {
        userRepository.findByUsername(user.getUsername())
                .ifPresentOrElse(
                        user1 -> user1.setRefreshToken(refreshToken),
                        () -> new Exception("회원이 없습니다")
                );
        refreshTokenRepository.findByRefreshToken(refreshToken.getRefreshToken())
                .ifPresentOrElse(
                        refreshToken1 -> refreshToken1.setUser(user),
                        () -> new Exception("리프레시 토큰이 없습니다")
                );
    }
